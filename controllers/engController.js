...
import { detailCommon, detailImage, detailIntro, detailInfo } from "../assets/detail/engDetailData";
import { mulType, categoryName, areaName, areaCode, searchResult } from "../assets/Variable";

export const getHome = (req, res) => {
  const areaArray = areaName.eng;
  const categoryArray = categoryName.eng;

  return res.send({ areaCode, mulType, areaArray, categoryArray });
};

export const getEngList = async (req, res) => {
  ...
  for (var i = 0; i < mulType.path.length; i++) {
    if (pathname.includes(mulType.path[i])) {
      await engApi
        ...
        .then((response) => {
          const engBase = response.data.response.body.items.item;
          ...
          const pageOfItems = engBase.slice(0, 12);

          const lang = "/eng";
          const addedAreaCode = area;
          const title = categoryName.eng[i];
          const areaArray = areaName.eng;
          const categoryArray = categoryName.eng;
...
export const getEngAreaDatas = async (req, res) => {
...
export const getEngDetailAll = (req, res) => {
  const pathname = req._parsedUrl.pathname;
